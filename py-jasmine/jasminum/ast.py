...
from jasminum.exceptions import JasmineEvalException
from jasminum.j import JFn, JType
...
def eval(source_code: str, source_id: int, engine: Engine, ctx: Context):
    nodes = parse_source_code(source_code, source_id)
    for node in nodes:
        eval_node(node, engine, ctx, False)
...
def eval_fn(fn: JFn, engine: Engine, ctx: Context, source_id: int, start: int, *args):
    ...
    if missing_arg_num == 0 and fn.arg_num == len(args):
        if isinstance(fn.fn, Callable):
            ...
            return eval_node(fn.fn, engine, fn_args, True)
    else:
        new_fn = copy(fn)
